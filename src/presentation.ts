...
function health(view: FusionView): "invalid-config" | "unavailable" | "low-confidence" | "ready" {
  ...
  if (view.recommendation && !view.recommendation.profile) return "low-confidence";
  return "ready";
}
...
export function formatStatus(view: FusionView): string {
  ...
  if (!view.recommendation.profile) {
...
export function formatExplain(view: FusionView): string {
  ...
  const rejected = view.recommendation.evaluations
    ...
    .join("; ") || "none";
  const route = view.recommendation.profile
    ? `recommended ${view.recommendation.profile}/${view.recommendation.modelId}`
    : "low-confidence; current model retained";
...
export function formatHistory(records: readonly TelemetryRecord[]): string {
  ...
  const lines = records.map((record) => {
    const route = record.recommendedProfile ?? "current-model-retained";
    return `${record.timestamp} · ${record.phase} → ${route} · ${percent(record.confidence)} · ${record.outcome} · active ${record.activeModel ?? "unknown"}`;
  });
...
export function formatConfig(view: FusionView): string {
  ...
  return `fusion config: shadow · ready · path ${view.config.path} · provider ${config.provider.id} · profiles ${Object.keys(config.profiles).length} · aliases ${Object.keys(config.aliases).length} · discovery ${discovery} · API key env reference configured (value hidden)`;
}
